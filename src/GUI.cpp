...
void GUI::render() {
    ...
    ImGui::Begin("Основное окно", nullptr, ImGuiWindowFlags_NoCollapse);
    ...
    if (ImGui::BeginTabBar("MainTabBar")) {
        ...
        for (auto panel = panels.begin(); panel != panels.end();) {
          ...
          std::string name="Вкладка " + std::to_string(numTab);
           ...
           if (ImGui::BeginTabItem(name.c_str(),&(*panel)->getIsOpen(), ImGuiTabItemFlags_UnsavedDocument)) {
...
void GUI::addEmployeePanel() {

    auto newListPanel = std::make_unique<EmployeeListPanel>(db);
...
//    newPanel->setEmployee(Employee{0, "", "", 0.0});  // Новый сотрудник
    ...
    newListPanel->getIsOpen()=true;
    newEditPanel->getIsOpen()=true;

    panels.push_back(std::move(newListPanel));
...
void GUI::showMainMenu() {
    if (ImGui::BeginMainMenuBar()) {
        ...
        if (ImGui::BeginMenu("Файл")) {
            ...
            if (ImGui::MenuItem("Выход")) { /* Закрыть приложение */ }
            ImGui::EndMenu();
        }
